...
if __name__ == '__main__':
    ...
    newspaper.add(item=washington_post)
    newspaper.add(item=new_york_times)
    newspaper.add(item=financial_times)
    ...
    search_engine.add(item=yahoo)
    search_engine.add(item=google)

    link_page = factory.create_page(title='LinkPage', author='James Smith')
    link_page.add(item=newspaper)
    link_page.add(item=search_engine)
